...
function wanxiang.is_mobile_device()
    ...
    local function _is_mobile_device()
        -- 获取系统信息
        local dist = rime_api.get_distribution_code_name() or ""  -- 获取发行版代码名称
        local user_data_dir = rime_api.get_user_data_dir() or ""  -- 获取用户数据目录
        local sys_dir = rime_api.get_shared_data_dir() or ""  -- 获取共享数据目录
        -- 转换为小写以便比较，不区分大小写
